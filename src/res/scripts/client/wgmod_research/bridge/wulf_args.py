...
Extracted from gameface_bridge.py so the parsing is engine-free and unit-testable
(Wulf delivers a single MAP argument per command; the JS side wraps a scalar as
{value: id}). gameface_bridge re-imports these under their old private names, so its
call sites are unchanged. Best-effort and side-effect-free; a plain dict, a
Wulf-wrapped map (any object with a .get(key)), or a bare scalar are all tolerated.
